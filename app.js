...
async function loadGridPressure(dateValue, options = {}, requestId = state.loadRequestId) {
  ...
  try {
    ...
    if (!demandPoints.length && !generationMix) {
      throw new Error("No hay datos de demanda o generación útiles.");
    }
    ...
    state.grid = {
      ...
      generationMix: null,
      cachedAt: "",
      reason: error.message || "No se pudieron cargar demanda y mix de generación.",
    };
  }
...
async function fetchDailyGridPayload(kind, dateValue, options = {}) {
  ...
  if (payload.errors?.length) throw new Error(payload.errors[0].detail || "REE devolvió un error.");
...
function hourlyDemandForSeries(series) {
  ...
  series.attributes.values.forEach((item) => {
    ...
    const current = hourly.get(hour) || {
      ...
      label: series.attributes.title || series.type || "Demanda",
    };
    current.total += value;
    current.count += 1;
    hourly.set(hour, current);
...
function renderGridPressure(best) {
  ...
  if (state.grid.status === "loading") {
    ...
    els.gridPressureTitle.textContent = "Comprobando presión de la red";
    els.gridPressureReason.textContent =
      "Cargando demanda y mix de generación de REE para estimar si esta ventana coincide con más presión de gas.";
    updateGridMetricText("--", "--", "--", "--");
    els.gridPressureMeta.textContent = "Fuente pendiente";
    return;
  }
  ...
  if (state.grid.status !== "ready") {
    ...
    els.gridPressureReason.textContent =
      state.grid.reason || "No hay datos suficientes de demanda y generación para explicar esta fecha.";
    updateGridMetricText("--", "--", "--", "--");
    els.gridPressureMeta.textContent = "La recomendación de precio sigue usando la serie PVPC/spot disponible.";
    return;
  }
...
function gridPressureSignal(best) {
  ...
  const demandText = windowDemand ? formatMw(windowDemand) : "demanda no disponible para esta ventana";

  if (pressureScore >= 0.66) {
    return {
      level: "high",
      demandMw: windowDemand,
      title: "Presión alta: el gas puede pesar más",
      reason:
        `En esta ventana la demanda ronda ${demandText}. El ciclo combinado representa ${gasText} del mix diario y las renovables ${renewableText}. ` +
        `Nuclear aparece como ${nuclearText}, pero se trata más como base estable que como señal de pico. Esta lectura sugiere más probabilidad de horas caras si la demanda sube o cae la solar.`,
    };
  }
  ...
  if (pressureScore >= 0.4) {
    return {
      level: "medium",
      demandMw: windowDemand,
      title: "Presión media: mira precio y demanda juntos",
      reason:
        `La demanda estimada de la ventana es ${demandText}. El ciclo combinado pesa ${gasText} y las renovables ${renewableText} en el día. ` +
        `La señal no apunta a excedente claro ni a estrés extremo, así que el precio horario sigue siendo el criterio principal.`,
    };
  }
  ...
  return {
    level: "low",
    demandMw: windowDemand,
    title: "Presión baja: mejor encaje para consumo flexible",
    reason:
      `La ventana coincide con menor presión relativa de demanda (${demandText}) y un mix diario con ${renewableText} renovable. ` +
      `El ciclo combinado está en ${gasText}. Es el tipo de señal donde mover coche, lavadora o climatización suele tener más sentido.`,
  };
}
...
function gridPressureMetaText() {
  ...
  return `Demanda real/prevista + mix diario de generación, ${source}${updated}. Señal orientativa, no marginalidad exacta.`;
}
...
function costHintText(kw, duration) {
  ...
  const fixedText = settings.socialBonusDaily
    ? `. Bono social fijo: ${formatMoney(socialBonusDailyAfterVat(settings))}/día después de ${taxLabel}; no cambia la mejor hora`
    : "";
  return `${kw.toFixed(1)} kW durante ${duration} h con ${settings.vat}% ${taxLabel}, ${settings.electricityTax}% impuesto eléctrico y ${formatEurKwh(settings.adder)} añadidos${fixedText}`;
}
